#!/bin/bash
#
# Create the TSTool Synoptic plugin product 'index.html' page on Amazon S3,
...
#
# software.openwaterfoundation.org/tstool-synoptic-plugin/index.html
# software.openwaterfoundation.org/tstool-synoptic-plugin/index.csv
#
...
#   2018-12-04 16:16:22          0 tstool-synoptic-plugin/
#   2018-12-04 16:16:37          0 tstool-synoptic-plugin/1.0.0/
#   2018-12-04 16:17:19   46281975 tstool-synoptic-plugin/1.0.0/tstool-synoptic-plugin-win-1.0.0-2201020304.zip
createCatalogFile() {
  ...
  if [ ${errorCode} -ne 0 ]; then
    logError ""
    logError "Error listing TSTool Synoptic plugin download files to create catalog (maybe missing --aws-profile)?"
    exit 1
  fi
  ...
  tmpS3CatalogPath="/tmp/${USER}-tstool-synoptic-plugin-catalog-ls-installers.txt"
  # Only list installers that match the current download filename specification
  cat ${tmpS3ListingPath} | grep -E 'tstool-synoptic-plugin.*.zip' > ${tmpS3CatalogPath}
  logInfo "Available TSTool Synoptic plugin installers:"
...
<p>
The TSTool Synoptic Plugin software is available for Windows.
...
<ol>
<li> Download a TSTool Synoptic Plugin Windows installer (zip file) from below.</li>
<li> Follow the latest <a href="https://software.openwaterfoundation.org/tstool-synoptic-plugin/latest/doc-user/appendix-install/install/">installation instructions</a>
...
<p>Linux versions of the TSTool Synoptic Plugin are not actively developed.
    ...
    cat "${tmpS3CatalogPath}" | grep "${downloadOs}-" | awk '{ printf "%s %s %s %s\n", $1, $2, $3, $4 }' | sed -E 's|([0-9][0-9]/)|\1-zzz|g' | sed 's|/-zzz|-zzz|g' | sed 's|dev|-dev|g' | sort -r -k4,4 | sed 's|-zzz||g' | sed 's|-dev|dev|g'  | awk '
      {
        ...
        downloadFileProduct=""            # The product name (e.g., TSTool Synoptic Plugin)
        ...
        if ( filenameSpecVersion = 1 ) {
          # Initial filename spec using format:  tstool-synoptic-plugin/1.0.0.dev/software/tstool-synoptic-plugin-1.0.0.dev-win-2201020304.zip
          ...
          downloadFileProduct="tstool-synoptic-plugin"
          downloadFileVersion=downloadFileParts[4]
          downloadFileOs=downloadFileParts[5]
        }
        # Set the URL to the download file.
        downloadFileUrl=sprintf("https://software.openwaterfoundation.org/tstool-synoptic-plugin/%s/software/%s", downloadFileVersion, downloadFile)
        product = "TSTool Synoptic Plugin"
        ...
        docDevUrl=sprintf("https://software.openwaterfoundation.org/tstool-synoptic-plugin/%s/doc-dev/",downloadFileVersion)
        ...
        docUserUrl=sprintf("https://software.openwaterfoundation.org/tstool-synoptic-plugin/%s/doc-user/",downloadFileVersion)
...
printUsage() {
  ...
  echoStderr ""
  echoStderr "Create the software.openwaterfoundation.org/tstool-synoptic-plugin/index.html and related files."
...
printVersion() {
  ...
  echoStderr ""
  echoStderr "TSTool Synoptic Plugin"
...
uploadIndexFiles() {
  ...
  logInfo "Assuming that TSTool Synoptic Plugin installer files were previously uploaded to S3."
  ...
  if [ -f "${s3CatalogTxtFileUrl}" ]; then
    ...
    if [ ${errorCode} -ne 0 ]; then
      logError ""
      logError "Error uploading TSTool Synoptic Plugin catalog file."
      # Not fatal.
    fi
  else
  ...
  #${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths "/tstool-synoptic-plugin/index.html*" "/tstool-synoptic-plugin/" "/tstool-synoptic-plugin" --profile "${awsProfile}"
  #${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths "/tstool-synoptic-plugin/index.html*" --profile "${awsProfile}"
  if [ "${operatingSystem}" = "mingw" ]; then
...
user=""
tmpS3ListingPath="/tmp/${USER}-tstool-synoptic-plugin-catalog-ls.txt"

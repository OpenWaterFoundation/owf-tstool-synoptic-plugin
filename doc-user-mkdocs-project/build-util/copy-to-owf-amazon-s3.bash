...
dryrun=""
#dryrun="--dryrun"
s3VersionFolder="s3://software.openwaterfoundation.org/tstool-synoptic-plugin/${pluginVersion}/doc-user"
s3LatestFolder="s3://software.openwaterfoundation.org/tstool-synoptic-plugin/latest/doc-user"
...
if [ -n "${pluginVersion}" ]; then
  ...
  ${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths "/tstool-synoptic-plugin/${pluginVersion}/doc-user/*" --output json --profile "${awsProfile}" | tee ${tmpFile}
...
if [ "${answer}" = "y" ]; then 
  ...
  if [ "${operatingSystem}" = "mingw" ]; then
    ...
    MSYS_NO_PATHCONV=1 ${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths "/tstool-synoptic-plugin/latest/doc-user/*" --output json --profile "${awsProfile}" | tee ${tmpFile}
  else
    ${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths "/tstool-synoptic-plugin/latest/doc-user/*" --output json --profile "${awsProfile}" | tee ${tmpFile}
  fi
  errorCode=${PIPESTATUS[0]}
  if [ $errorCode -ne 0 ]; then
    logError " "
